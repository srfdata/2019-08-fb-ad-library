#!/bin/bash
# knit
# add >>>export PATH="$PATH:/usr/lib/rstudio/bin/pandoc:$PATH"<<< to your PATH in order for knitting to work (Linux)
# add >>>export PATH="/Applications/RStudio.app/Contents/MacOS/pandoc:$PATH"<<< to your path on Mac
...
/opt/R/R-3.5.3/bin/R -e 'Sys.unsetenv("RSTUDIO"); library(rmarkdown); rmarkdown::render("analysis/main.Rmd", "html_document")' --no-site-file --no-init-file --no-restore --no-save || { echo "ERROR: knitting failed."; exit 1; }
