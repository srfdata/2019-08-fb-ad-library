...
* Nach dem Crawl-Vorgang werden die API-Antworten im vorliegenden Script eingelesen, vorprozessiert und ausgewertet. Dabei werden verschiedene Grafiken erstellt und die aufbereiteten Daten in ein CSV gegossen (siehe Datenbeschreibung).
* Für die Auswertungen werden aus Gründen der Vergleichbarkeit jeweils nur die Daten der kantonalen und nationalen Partei-Sektionen (=API-Resultate, die auf Anfrage einer Page-ID retourniert wurden) verwendet.
...
## Daten einlesen vorprozessieren
...
#### Minimal- und Maximalausgaben nach Page / "Funding Entity", Partei

Alle Angaben in CHF.

```{r minimally and maximally spent per page/funding entity, echo=TRUE, message=FALSE, warning=FALSE}
ads_to_analyze %>% 
  group_by(page_name) %>% 
...
ads_to_analyze %>% 
  ...
  ggplot(aes(x = week_number, y = week_ads)) +
